...
> hardware yet, so expect to iterate the Dockerfile and launches on first
> run.
...
- For the sim GUI on Windows: VcXsrv (the `start_sim.sh` script starts it
  with `-wgl`).
- For the UI: host Python 3.9+ (a venv is created automatically by
  `run_ui.sh`).
...
elsewhere, adjust the path. From a Git Bash prompt you can drop the prefix
and just run `bash ./start_sim.sh`. The real-robot commands run on the Pi
(Linux), where it's always plain `bash ./start_real.sh`.
...
You should see Gazebo with the Bunker in the minibunker arena, complete with
a green ball and orange cones. The rover boots disarmed, so it won't move
until you arm it (see §3). Gazebo's camera plugin publishes
`/camera/image_raw`, and the Bunker's planar-move plugin consumes `/cmd_vel`
and emits `/odom`.
...
The `image_pub` node can feed a synthetic scene (a moving green ball and a
cone) or a video file into `/camera/image_raw`, so the whole detect-then-
behave stack runs on any laptop with nothing attached:
...
- Arm or disarm the safety gate. The rover publishes zero velocity until
  armed.
- Switch the detector backend, HSV or CNN, live.
- Tune HSV ranges, CNN confidence, follow gain, speed, stop distance, cone
  danger size, and the speed caps, all applied live (the nodes re-read these
  "soft" knobs every frame).
...
See [HARDWARE_SETUP.md](HARDWARE_SETUP.md) for the Pi 5, camera, and CAN
setup, then on the Pi (Linux, plain `bash`, since the Windows `bash.exe` path
doesn't apply here):
...
option A (rosbridge) is the default, but it's documented here as the
fallback to implement if you need it.
