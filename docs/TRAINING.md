...
This covers the "how a robot learns to see" pipeline: annotate, train, export,
deploy. The HSV baseline is the "before"; the trained YOLOv8-nano is the
"after." It runs off-robot, on Colab or any CUDA box.
...
- Public Roboflow Universe. Search `traffic cone` / `construction cone` and
  ...
  ```
- Annotate real arena photos (recommended). Shoot roughly 150 to 300 photos of
  the actual ball and cones under the real arena lighting and background,
...
If a source dataset uses different class names or ordering, remap the labels
so green_ball is 0 and cone is 1 before training.
...
don't want to distort green versus orange. Aim for a model that runs at least
10 FPS on the Pi 5 CPU.
